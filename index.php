...
<table table style="width:66%;" class="table table-sm table-borderless table-striped text-center mx-auto mt-3" >
  <thead class="bg-dark text-white text-center">
  <tr>
    <th>Serial</th>
    <th>task</th>
...
<?php
require_once "db.php";
$task_show_query = "SELECT * FROM deleted ORDER BY id DESC LIMIT 10";
$result = $dbcon -> query($task_show_query);
if($result->num_rows!=0){
  $serial = 1;
  foreach ($result as $row) {
